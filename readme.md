...
Для работы необходимо запустить телеграм бота и отправить сообщение "ПогодаАрх". В ответ бот отвечает сообщением, которое содержит в себе текущую дату, фактическую температуру, ощущаемую температуру и ссылку на сайт Яндекс погоды для подробной информации.

Способ получения токена: 

1. Заходим на сайт https://developer.tech.yandex.ru/
2. Заполняем антеку для генерации токена, и получаем токен по выбранному тарифу (был использован "погода на вашем сайте")
3. Для взаимодействия с api необходимо ввести полученный токен в пользовательском режиме в константу "ТокенСервисаПогодыАрх"
